      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="hidden lg:flex items-center gap-6">
            {navLinks.map(link => (
              <Link key={link.to} to={link.to} className="text-sm font-body font-bold text-gray-700 hover:text-primary-600 transition-colors duration-200">
                {link.label}
              </Link>
            ))}
          </nav>
          ...
          <div className="flex items-center gap-2">
            {user ? (
              <div className="flex items-center gap-2">
                <Link to={user.role === 'owner' ? '/owner' : '/track'} className="hidden sm:flex items-center gap-1 text-sm font-body font-bold text-gray-700 hover:text-primary-600 transition-colors">
                  <FiUser size={16} />
