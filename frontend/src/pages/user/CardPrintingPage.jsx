      ...
      <div className="max-w-3xl mx-auto px-4">
        ...
        {step === 3 && selectedTemplate && (
          <div className="card p-6 fade-in-up space-y-5">
            ...
            <div>
              ...
              <input className="input-field" type="email" value={customer.email} onChange={e => setCustomer(p => ({ ...p, email: e.target.value }))} placeholder="your@email.com (optional)" />
            </div>
        ...
        {step === 4 && (
          <div className="card p-6 fade-in-up space-y-6">
            ...
            <div className="flex gap-3">
              <button onClick={() => setStep(3)} className="btn-outline flex-1">← Back</button>
              <button disabled={loading || !captchaOk} onClick={handleSubmit}
                className={`flex-1 font-bold py-3 px-6 rounded-lg flex items-center justify-center gap-2 ${captchaOk ? 'btn-primary' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}>
                {loading ? <><div className="loader w-4 h-4 border-2" /> Submitting...</> : '✅ Submit Order'}
              </button>
            </div>
          </div>
