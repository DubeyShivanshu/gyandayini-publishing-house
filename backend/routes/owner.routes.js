...
// PUT /api/owner/requests/:requestId/status
...
// PUT /api/owner/requests/:requestId/receipt
router.put('/requests/:requestId/receipt', async (req, res) => {
  try {
    ...
    const request = await Request.findOneAndUpdate(
      { requestId: req.params.requestId },
      { receipt: { items: processedItems, grandTotal, notes: notes || '', generatedAt: new Date() } },
      { new: true }
    );
    if (!request) return res.status(404).json({ success: false, message: 'Request not found' });
...
// NEW: DELETE /api/owner/requests/:requestId
router.delete('/requests/:requestId', async (req, res) => {
  try {
    const request = await Request.findOneAndDelete({ requestId: req.params.requestId });
    if (!request) return res.status(404).json({ success: false, message: 'Request not found' });
...
// NEW: DELETE /api/owner/requests (delete all or filtered)
router.delete('/requests', async (req, res) => {
  try {
    ...
    res.json({ success: true, message: `${result.deletedCount} request(s) deleted` });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
});
